...
int main(int argc, char *argv[]) {
  ...
  constexpr bool plotField = true;
   ...
   static constexpr double fGasGapThickness = 0.025;         // cm - total gas gap thickness            250 micron = 0.25 mm = 0.025 cm
    ...
    static constexpr double fDetectorSizeX = 30.0/10.0;         // cm - detector size in X                 30 mm = 30 cm    
    static constexpr double fDetectorSizeZ = 30.0/10.0;         // cm - detector size in Z                 30 mm = 30 cm
    ...
    static constexpr double fAnodeVoltage = 5500.0;               // V - ANODE at +2.5 kV
    static constexpr double fCathodeVoltage = -5500.0;            // V - CATHODE at -2.5V
...
gas.Initialise(true);
  ...
  int count = 0;
  int total_electrons_per_cluster = 0;
  int totalElectrons = 0;
  int totalClusters = track.GetClusters().size();
  for (const auto &cluster : track.GetClusters()) {
    // Loop over the electrons in the cluster.
     int clusterElectrons = cluster.electrons.size();
     totalElectrons += clusterElectrons;
     count++;
     std::cout << "Cluster: " << count << "/" << totalClusters
         << " | Electrons in cluster: " << clusterElectrons
         << " | Total electrons so far: " << totalElectrons
         << "\r" << std::flush;
