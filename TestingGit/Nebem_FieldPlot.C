...
int main(int argc, char *argv[]) {
  ...
  constexpr bool plotField = true;
   ...
   static constexpr double fGasGapThickness = 0.025;         // cm - total gas gap thickness            250 micron = 0.25 mm = 0.025 cm
    ...
    static constexpr double fAnodeVoltage = 5500.0;               // V - ANODE at +5 kV
    static constexpr double fCathodeVoltage = -5500.0;            // V - CATHODE at -5V
...
gas.Initialise(true);
 ...
 fieldView->PlotProfile(0,-0.025,0,0,0.025,0,"emag",true);
  ...
  contourView->SetPlane(0,-1,0,0,0.015,0);
